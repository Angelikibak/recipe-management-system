...
import recipes.database.RecipeIngredientRepository;
...
public class ManageRecipeIngredientsFrame extends JFrame {

    private final int recipeId;

    private final IngredientRepository ingredientRepo = new IngredientRepository();
    private final RecipeIngredientRepository recipeIngredientRepo = new RecipeIngredientRepository();
    ...
    public ManageRecipeIngredientsFrame(int recipeId) {
        this.recipeId = recipeId;

        setTitle("Manage Recipe Ingredients (Recipe ID: " + recipeId + ")");
        setSize(800, 450);
    ...
    private void buildUI() {
        ...
        left.add(new JLabel("Ingredients for this recipe"), BorderLayout.NORTH);
    ...
    private void loadList() {
        try {
            listModel.clear();
            List<StepIngredient> items = recipeIngredientRepo.findByRecipeId(recipeId);
    ...
    private void save() {
        try {
            ...
            recipeIngredientRepo.upsert(recipeId, ing.getId(), qty, unit, note);
    ...
    private void delete() {
        try {
            ...
            int confirm = JOptionPane.showConfirmDialog(this,
                    "Remove ingredient '" + si.getIngredientName() + "' from this recipe?",
                    "Confirm delete",
                    JOptionPane.YES_NO_OPTION);

            if (confirm == JOptionPane.YES_OPTION) {
                recipeIngredientRepo.delete(recipeId, si.getIngredientId());
                loadList();
                clearForm();
                selected = null;
                ingredientList.clearSelection();
